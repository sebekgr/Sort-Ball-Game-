...
const chooseBall = (target) => {
    const parent = target.parentElement;

    if (!$activeBall) {
        if (
            parent.childElementCount > 0 &&
            parent.firstElementChild.classList.contains("ball")
        ) {
            ...
            target.firstElementChild.classList.toggle("ball-active");
            $activeBall = e.target.firstElementChild;
        }
    } else {
        moveBallToSelectedRectangle(target);
    }
};

const moveBallToSelectedRectangle = (target) => {
    if (
        target === $activeBall ||
        target === $activeBall.parentElement.children[1] ||
        target === $activeBall.parentElement.children[2]
    ) {
    ...
    if (
        ...
        if (
            ...
            target.children[0].title === $activeBall.title
        ) {
            ...
            activeBall.classList.replace("ball-active", "ball-error")
        }
        $activeBall = null;
    } else if (
        target.parentElement.classList.contains("rectangle") &&
        target.parentElement.childElementCount < 3
    ) {
        if (
            target.parentElement.children.length === 0 ||
            target.parentElement.children[0].title === $activeBall.title
        ) {
            const el = document.createElement("div");
...
const checkingSortBalls = () => {
    ...
    for (let i = 0; i < countParents; i++) {
        ...
        if (element.childElementCount === 3) {
            ...
            if (
                titleBall === element.firstElementChild.title &&
                titleBall === element.lastElementChild.title
            ) {
                $score++;
